...
// needCalls 为了times调用后，才调用callback
// 如果一报错，直接调用callback
...
class NormalModuleFactory extends ModuleFactory {
	constructor({ context, fs, resolverFactory, options }) {
		...
		this.hooks.factorize.tapAsync(
			...
			(resolveData, callback) => {
				// resolve的处理结果，会绑定一个data.createData 值在resolveData上
		...
		this.hooks.resolve.tapAsync(
			...
			(data, callback) => {
				...
				// 会调用 ResolverFactory 实例的get方法，本质是
				// 针对不同type，resolveOptions，利用webpack/enhanced-resolve 库，构建不同resolver
				// resolveOptions 其实就是对应options中传入的resolve参数
				...
				// 对于normalResolver 获取路径
				// 如elements = 0 ，则上面this.resolveRequestArray，会将times-- ，此处normalResolver会执行continueCallback
				// 对于正常的路径文件，也需要用一些loader来处理
	...
	resolveRequestArray(
		...
		// async 的代替品neo-async
		// .map 方法，就是array，分别用iteratee处理，全处理完调用callback
		asyncLib.map(
			array,
			(item, callback) => {
				resolver.resolve(
	...
	createGenerator(type, generatorOptions = {}) {
		// 不同的type类型，会在JavascriptModulesPlugin.js return generator 新实例，
		// 目前都是JavascriptGenerator.js
