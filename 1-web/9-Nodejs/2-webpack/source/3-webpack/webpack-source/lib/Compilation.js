...
class Compilation {
	...
	constructor(compiler) {
		...
		/** @type {Set<Chunk>} */
		// 可以说与namedChunks配对使用，此存 new Chunk(name), namedChunk存 name
		this.chunks = new Set();
	...
	_processModuleDependencies(module, callback) {
		// 理解为缓存dep.constructor 的factory的
		// { factory: { resourceIdent: [ dep ] } }
	...
	/**
	 * 处理dependency，首先判断dependency是否为webpack支持的类型，如amd，commonjs等
	...
	addModuleChain(context, dependency, callback) {
		...
		// Todo 1、dependencyFactories何时赋值的
		// 2、这个dependency具体指什么
	...
	seal(callback) {
		...
		for (const [name, { dependencies, options }] of this.entries) {
			// 在namedChunks 保存name， 在chunks 保存 new Chunk(name)
			// ChunkGraph.setChunkGraphForChunk(chunk, this.chunkGraph);
			// 每个entry，都会通过addChunk方法创建一个chunk
		...
		// 用于生成并优化 chunk 依赖图，建立起各模块之前的关系
		// 从Module graph生成Chunk graph
		buildChunkGraph(
			this,
		...
		// RuntimeChunkPlugin，options配置了optimization.runtimeChunk.name = bundle
			...
			this.hooks.optimizeChunkModules.callAsync(
				...
				err => {
					...
					const cont = () => {
						...
						// 为this.fileDependencies，this.contextDependencies 等赋值
			...
			(chunk, callback) => {
				...
				asyncLib.forEach(
					...
					(fileManifest, callback) => {
						// hotUpdateChunk ? `hotupdatechunk${chunk.id}` : `chunk${chunk.id}`
