...
const extractBlockModulesMap = compilation => {
	...
	for (const module of compilation.modules) {
		...
		let moduleMap;
		// 获取module.outgoingConnections 值
...
const visitModules = (
	...
	// 处理module.outgoingConnections  应该是获取 异步引用模块与modules关系？？？？？？？？？？
	const blockModulesMap = extractBlockModulesMap(compilation);

	let nextChunkGroupIndex = 0;感觉
	...
	const iteratorBlock = b => {
		...
		if (cgi === undefined) {
			...
			// 如cgi存在，则会抛出异常，即name already used
			if (!cgi) {
 ...
 * @param {Compilation} compilation the compilation
 * @param {Entrypoint[]} inputEntrypoints chunk groups which are processed  inputEntrypoints = this.chunkGroups.slice()(
 * @returns {void}
 */
