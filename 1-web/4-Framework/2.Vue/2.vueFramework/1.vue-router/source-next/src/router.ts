...
export function createRouter(options: RouterOptions): Router {
  ...
  function pushWithRedirect(
    ...
    // todo redirectedFrom??????
      ...
      // 同路由报错 all `params`, `query` parameters and `hash` are the same
  ...
  // TO: refactor the whole before guards by internally using router.beforeEach
  ...
  function navigate(
    ...
    let guards: Lazy<any>[]
    // 1. 根据 from与to的路由信息表，获取update，leave，enter，具体内容
    const [
      leavingRecords,
      updatingRecords,
      enteringRecords,
    ...
    // pendingLocation !== to  reject(error)
  ...
  function setupListeners() {
    removeHistoryListener = routerHistory.listen((to, _from, info) => {
      ...
      // TO: should be moved to web history?
      if (isBrowser) {
        saveScrollPosition(
