...
#### 重排序方法
1. reverse()：反转数组项顺序
2. `sort(values，function(value1,value2){})`:
  - 按照某种方式对数组进行排序，可以自定义比较函数;
  - 默认情况下，会调用values数组每一项的tostring方法，然后比较字符串
...
5. `foreach()`
  ...
  - 注意：`forEach`不直接改变调用它的对象，但是对象可能会被`callback`改变
...
2. 数量词
	...
	| *x*(?=*y*)      | 只有当 `x` 后面紧跟着 *y` 时，才匹配 *x* | /Jack(?=Sprat)/<br /><br />"JackSprat"<br />匹配结果是：Jack |
	| *x*(?!*y*)      | 不紧邻时匹配                             |                                                              |
