...
class JavascriptModulesPlugin {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.hooks.chunkHash.tap(
					"JavascriptModulesPlugin",
					(chunk, hash, context) => {
						// tap 4 func, 实际就是将各种信息 加入 hash 内容中
	...
	renderMain(renderContext, hooks) {
		...
		// 在complication， processRuntimeRequirements阶段，会为cgc.runtimeRequirementsInTree赋值
		// 实际获取的是，生成代码中，用到的类似：__webpack_require__.e，__webpack_require__.r
		const runtimeRequirements = chunkGraph.getTreeRuntimeRequirements(chunk);
		// runtimeTemplate.isIIFE() = this.outputOptions.iife、
		// 默认：true，Tells webpack to add IIFE wrapper around emitted code.
