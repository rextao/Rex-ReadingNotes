# 类型
## 1、内置类型
1. 内置类型包括5种基本类型（undefined，Null，Boolean，Number，String，symbol（ES6新增））与复杂类型（object）
2. 所有值都是上述6种类型之一
...
2. 从技术角度看，函数在ECMAScript中是对象，不是一种数据类型(虽然返回是function);
```javascript
typeof function(){} ==="function"  // true
```
1. null的特殊情况
	- 逻辑角度看，null值表示的是一个空对象指针，所以typeof会返回Object
	- 这是一个bug，并且标准无法修复，因为涉及web页面太多
```javascript
...
1. 内部属性 [[Class]]
	...
	- 这个属性无法直接访问，需要Object.prototype.toString(..) 来查看，如
```javascript
...
## 6、Number类型
### 概述
1. js没有真正意义的整数，只有没有小数的十进制数
2. 数字值可以用Number对象进行封装(js引擎会做自动装箱)，故数值可直接调用Number的方法
3. （.运算符）会优先识别为数字常量一部分,基本不会如此调用
```javascript
...
### 浮点数
1. 浮点数需要的空间是整数的2倍，故ECScript会进行转换，对于如1.0，10.000这样数会转换为1，,10
...
2. object类型分为数组、函数和内置对象，内置对象包括，String，Number，Date等，如"hello world"为String类型，能调用indexof方法是因为，js有sring的内置对象，hello world继承自这个内置对象
2. 在ES中，Object类型是所有它的实例的基础，但并不适用于js其他对象，比如dom或bom中的对象

### Object类型
#### 类型本身
1. 概述
 - Object类型本身不具备多少功能（如只有toString,isPrototypeOf等），但对于在应用程序中存储和传输数据而言，是非常理想的选择
 - 对象中，属性名永远是字符串，非字符串会被转为字符串
1. 创建Object实例的方式
    - new Object()
    - 对象字面量表示法：var person = { name:"tom",age:29}
    - 两者唯一区别是：构造形式需要逐个添加属性
1. 访问属性
    - person.name
    - person["name"]
	- 方括号语法的优点是可通过变量来访问属性
...
1. 复制对象
	- 对于JSON安全对象（json双向解析对象是一模一样的）
		 - JSON.parse( JSON.stringify( someObj ) );
	- ES6提供Object.assign()进行浅复制
		- 使用=操作符来赋值，不能复制对象特性（如writable）

#### 对象
1. 概述
    - 面向对象的标志是有类的概念，js没有类的概念，故js对象与其他语言对象有所不同
	- 对象定义：无序属性集合，其属性可以包含基本值、对象或函数
1. 属性描述符
	- 概述
...
1. 对象不变性
	- ES5（下述方法） 所有的方法创建的都是浅不变性，即定义防篡改对象
		- 只会影响目标对象和它的直接属性
		- 对象定义为防篡改后，不能撤销
	- js深不变性很少需要
	- 对象常量
	...
	- 密封Object.seal(..)
		- 调用Object.preventExtensions(..) 并把所有现有属性标记为 configurable:false
		- 不能删除属性和方法，但属性值是可以改变的
		- Object.isSealed()：判断是否密封对象
	- 冻结Object.freeze(..)
		- 调用Object.seal()并把所有“ 数据访问” 属性标记为 writable:false
...
1. 检测数组
	- instanceof操作符（value instanceof Array）：对于一个网页或者全局作用域而言可用，但对于网页包含多个iframe，就会有两个不同的全局执行环境，两个不同版本的Array构造函数，就会出现问题
...
## 11、值和引用
### 简单值
1. 通过值复制的方式来赋值 / 传递: 不改变原值
1. 简单值有：null、 undefined、字符串、数字、布尔和 ES6 中的 symbol

##复合值
1. 通过引用复制的方式来赋值 / 传递：改变原值
2. 复杂值，如数组和封装对象
...
```
    ...
    如将标量基本类型值传递到函数内并进行更改，就需要将该值封装到一个复合值（对象、数组等）中，然后通过引用复制的方式传递
