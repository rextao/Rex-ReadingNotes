...
3. `==`或`!=`，只要两边不都是字符串，转为数字进行比较

	- 虽然进行转换数值时，undefined 转换为 NaN， null 转换为 0  ，但`undefined== null;// true`
	- null不等于其他任何值
	- NaN不等于任何值，包括自己
